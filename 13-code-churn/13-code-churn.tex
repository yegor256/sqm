...
\lnQuote
  [Sebastian G. Elbaum]
  {sebastian-elbaum}
  {We can measure the increase or decrease in system complexity as measured by a selected metric, \emph{code delta}, or we can measure the total amount of \emph{change} the system has undergone between builds, \textcolor{orange}{code churn}.}
  {munson1998}
...
\lnPitch{
  ...
  items \emph{moving out} of a group over a specific period.''
  \par\columnbreak\par
...
\lnPitch{
  ...
  \emph{doesn't} give an indicator as to how much change the
...
\lnPitch{
  ...
  \par\columnbreak\par
  ``In the case of code churn, what is important is the absolute measure of the amount of code that has been modified. From the standpoint of \emph{fault insertion} (FI), removing a lot of code is probably as catastrophic as adding a similar amount.''
  \par
...
\lnQuote
  [Nachiappan Nagappan]
  {nachiappan-nagappan}
  {Our case study provides strong support for the following conclusion: increase in relative code churn measures is accompanied by an increase in system \emph{defect density}.}
  {nagappan2005use}
...
\lnQuote
  [Georgios Gousios]
  {georgios-gousios}
  {We examined 291 carefully selected Ruby, Python, Java and Scala projects (in total, 166,884 pull requests)... The number of \emph{total lines} changed by pull requests is on average less than 500 (95\% percentile: \textbf{1227}, 90\% percentile: 497, 80\% percentile: 168) with a median number of 20.}
...
[Tobias Olsson]
  {tobias-olsson}
  {Non-normal code churn can be a tangible and measurable \emph{effect} of architectural violations in source code. However, more research is needed to better understand this connection, e.g., if violations are the \emph{cause}, the size of the effect, the cost of refactoring, and whether it is generalizable to other system.}
  {olsson2017}
