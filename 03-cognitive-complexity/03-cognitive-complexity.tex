...
\lnQuote
  [Bill Curtis]
  {bill-curtis}
  {\hs{Computational} complexity assesses the difficulty of verifying an algorithm's correctness, while \hs{psychological} complexity assesses human performance on programming tasks.}
  {curtis1979measuring}
...
\lnQuote
  [Lionel C. Briand]
  {lionel-briand}
  {By cognitive complexity we mean the \hs{mental burden} of the persons who have to deal with the class. We \hs{assume} that it is the, sometimes necessary, high cognitive complexity of a class which causes it to display undesirable external qualities, such as decreased maintainability and testability, or increased fault-proneness.}
  {briand2001integrating}

\lnQuote
  [Jingqiu Shao]
  {jingqiu-shao}
  {Cognitive complexity, the \hs{new measure} for software complexity presented in this paper, is a measure of the cognitive and psychological complexity of software as a human intelligence artifact.}
  {shao2003new}

\lnPitch{
  \begin{multicols}{2}
  \includegraphics[width=.7\linewidth]{bcs.png}\par
  BCS stands for ``\hs{basic control structure}.''
  \par\columnbreak\par
  \begin{spacing}{0.9}
...
\lnQuote
  {yingxu-wang}
  {Cognitive complexity of software is a product of its architectural and operational complexities on the basis of \hs{deductive semantics} and the \hs{abstract system theory}.}
  {wang2006cognitive}
...
\lnQuote
  [Ann Campbell]
  {ann-campbell}
  {A guiding principle in the formulation of Cognitive Complexity has been that it should incent \hs{good} coding practices. That is, it should either ignore or discount features that make code more readable.\pptPin{\includegraphics[width=7em]{sonarqube.pdf}}}
  {campbell2018cognitive}

\lnPitch{
  \pptPic{.9}{motive-before.png}\par
  ``The mathematical model underlying \hs{Cyclomatic Complexity} gives these two
...
\lnPitch{
  ...
  ``The \hs{Cognitive Complexity} algorithm gives these two methods markedly different
...
\lnQuote
  [Rub{\'e}n Saborido]
  {ruben-saborido}
  {Different cognitive complexity measures have been proposed to quantify the understandability of a piece of code and, therefore, its maintainability. \hs{However}, the cognitive complexity metric provided by SonarSource is quickly spreading in the software industry due to the popularity of their well-known static code \hs{tools} for evaluating software quality.}
...
\lnThought{Maybe it's impossible to accurately measure the ``\hs{perceived} complexity'' of software?}

\lnQuote
  {norman-fenton}
  {We showed that the search for \hs{general} software complexity measures is doomed to failure.}
  {fenton1994software}

\lnQuote
  [Luigi Lavazza]
  {luigi-lavazza}
  {Being able to find statistical correlations between code \hs{understandability} and source code \hs{measures} would be greatly beneficial for the software development process, which involves a great deal of activities involving program \hs{comprehension}.}
  {lavazza2023empirical}

\lnThought{Social code analysis: we analyze the \hs{author(s)} of the code, not only the code itself.}

\lnQuote
  {../03-cognitive-complexity/social}
  {Social code analysis enriches our understanding of the code quality by overlaying a \hs{developer's behavior} with the \hs{structural analysis} of the code.}
  {social2017}
...
\lnQuote
  {raul-barbosa}
  {We propose the use of \hs{biofeedback} code highlighting techniques to provide online programmer's warnings for potentially problematic code lines that may need a second look at (to remove possible bugs).}
...
\par
The ``cognitive state code annotations'' are on the right side of the code, as Java comments, indicating the level of \hs{brain activity} at the moment of writing the particular line of code.
\plush{}

\lnQuote
  {julio-medeiros}
  {The EEG results reveal evidence of \hs{mental effort saturation} as \hs{code complexity} increases. Conversely, the classic software complexity metrics do \hs{not accurately} represent the mental effort involved in code comprehension.}
  {medeiros2021can}
