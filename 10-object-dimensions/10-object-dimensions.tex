...
\lnQuote
  ...
  {fernando-brito-e-abreu}
  {\textbf{Metrics for Object-Oriented Design (MOOD)}: Being able to predict some software quality characteristics based on the design, is one of our great motivations. This ability will allow the designing process to be guided, for instance, by means of \hs{heuristics}.}
  {brito1994object}
...
``MHF is defined as the ratio of the sum of the \hs{invisibilities} of all methods
...
``AHF is defined as the ratio of the sum of the \hs{invisibilities} of all attributes
...
``MIF is defined as the ratio of the sum of the \hs{inherited} methods in all classes of
...
``AIF is defined as the ratio of the sum of \hs{inherited} attributes in all classes of
...
``PF is the number of methods that \hs{redefine} inherited methods, divided by the
...
\lnQuote
  [Chris F. Kemerer]
  {chris-kemerer}
  {The inheritance hierarchy, a directed acyclic graph will be described as a tree structure with classes as nodes, leaves and a root. In any design application, there can be many possible \hs{hierarchies of classes}. Design choices on the hierarchy employed to represent the application are essentially choices about restricting or expanding the scope of properties of the objects in the application.}
...
``DIT is a measure of how many \hs{ancestor} classes can potentially affect this class.''
\end{multicols}
\plush{}
...
``NOC is a measure of how many \hs{sub-classes} are going to inherit the methods of the parent class.''
\end{multicols}
\plush{}
...
``RFC is the count of the set of \hs{all methods} that can be invoked in response to a message to an object of the class or by some method in the class.''
\end{multicols}
\plush{}
