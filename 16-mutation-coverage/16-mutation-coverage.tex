...
%   [Jefferson A. Offutt]
%   {richard-hamlet.jpg}
%   {\scriptsize ``Dr. Hamlet presented an early testing system that was embedded in a compiler and performed a version of instrumented weak mutation. Although the method differed significantly from later mutation systems, Hamlet's system seems to be \emph{the first mutation-like testing system}.''}
%   {offutt1991}

\lnQuote
  [Richard J. Lipton]
  {richard-lipton}
  {Our groups at Yale University and the Georgia Institute of Technology have constructed a system whereby we can determine the extent to which a given set of test data has \emph{adequately} tested a Fortran program by direct measurement of the number and kinds of errors it is \emph{capable} of uncovering.}
  {demillo1978hints}

\lnQuote
  {timothy-budd}
  {A test set is \emph{adequate} if it can distinguish the subject program from a collection of similar programs, called mutants, obtained by making \emph{small} syntactic modifications to the subject program.}
  {budd1981mutation}
...
\lnQuote
  {william-howden}
  {In \emph{weak mutation testing} method, tests are constructed which are guaranteed to force program statements which contain certain classes of errors to act incorrectly during the execution of the program over those tests.}
  {howden1982weak}
...
\lnQuote
  [Jeff Offutt]
  {jeff-offutt}
  {Our results indicate that \emph{weak mutation} can be applied in a manner that is almost as effective as mutation testing, and with significant computational savings.}
  {offutt1994empirical}
...
\lnQuote
  [Richard DeMillo]
  {richard-demillo}
  {A mutant operator mutates \emph{one syntactic entity} of a program. Further, only one mutant operator
...
\lnQuote
  [Phyllis G. Frankl]
  {phyllis-frankl}
  {Those mutants that compute precisely the same function are called \emph{equivalent} mutants and the others are called \emph{inequivalent} mutants.}
  {frankl1997all}
...
\lnQuote
  [Lionel C. Briand]
  {lionel-briand}
  {Our analysis suggests that mutants, when using carefully selected mutation operators and after removing equivalent mutants, can provide a \emph{good indication} of the fault detection \emph{ability} of a test suite.}
  {andrews2005}
\lnPitch{\begin{multicols}{2}
...
\lnQuote
  [Yu-Seung Ma]
  {yu-seung-ma}
  {Comparing with previous mutation systems for procedural programs, \emph{MuJava} is very fast. However, it is relatively slow when it generates and runs lots of mutants.}
  {ma2006}
\lnPitch{\begin{multicols}{2}
...
\lnQuote
  [Mark Harman]
  {mark-harman}
  {Traditional mutation testing considers only first order mutants, created by the injection of a single fault. Often these first order mutants denote trivial faults that are easily killed. \emph{Higher order mutants} are created by the insertion of two or more faults.}
  {jia2009}
...
\lnQuote
  [Yue Jia]
  {yue-jia}
  {One \emph{problem} that prevents mutation testing from becoming a practical testing technique is the \emph{high computational cost} of executing the enormous number of mutants against a test set.}
  {jia2010}
...
\lnQuote
  [Ren{\'e} Just]
  {rene-just}
  {Our experiments used 357 real faults in 5 open-source applications that comprise a total of 321,000 lines of code. The results show a statistically \emph{significant} correlation between mutant detection and real fault detection, independently of code coverage.}
  {just2014mutants}
